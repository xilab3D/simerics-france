---
title: "Services"
description: "Licences, support, formation, prestations d'expertise CFD avec Simerics"
ShowToc: true
TocOpen: false
---

Simerics France propose un accompagnement complet autour de la suite logicielle Simerics.

<div class="cards-grid">

<div class="card accent">
<h3>Licences</h3>
<ul>
<li>Simerics-MP®</li>
<li>Simerics-MP+® (modules métiers)</li>
<li>PumpLinx®</li>
<li>Modules complémentaires</li>
<li>Licences perpétuelles ou annuelles</li>
<li>Floating / nodelock</li>
</ul>
<a class="card-link" href="/contact">Demander un devis →</a>
</div>

<div class="card accent">
<h3>Support technique</h3>
<ul>
<li>Hotline en français</li>
<li>Support de premier niveau</li>
<li>Réunions régulières définies sur l'année</li>
<li>Mises à jour incluses</li>
<li>Portail client</li>
</ul>
</div>

<div class="card accent">
<h3>Formation</h3>
<ul>
<li>Prise en main Simerics-MP/MP+</li>
<li>Modules métiers (PD, Turbo, Vehicle...)</li>
<li>Sur site ou à distance</li>
<li>Formations sur cas client</li>
<li>Certification possible</li>
</ul>
<a class="card-link" href="#formations">Voir le catalogue →</a>
</div>

<div class="card accent">
<h3>Prestations d'expertise</h3>
<ul>
<li>Études CFD sous-traitées</li>
<li>Études pilotes / faisabilité</li>
<li>Mise en place de templates</li>
<li>Automatisation de workflows</li>
<li>Audit de modèles existants</li>
</ul>
<a class="card-link" href="/contact">Discuter d'un projet →</a>
</div>

</div>

---

## Formations {#formations}

Toutes nos formations sont dispensées par des **ingénieurs CFD praticiens** ayant une expérience industrielle directe sur Simerics. Les durées sont indiquées à titre informatif, une réunion préalable sera nécessaire pour estimer les compétences initiales. 

| Formation | Durée | Format |
|-----------|-------|--------|
| **Prise en main Simerics-MP/MP+** | 1 jour | Sur site ou à distance |
| **Module PD : pompes volumétriques** | 2 jours | Sur site ou à distance |
| **Module Turbo : pompes centrifuges** | 2 jours | Avec intégration CFturbo |
| **Module Valves & systèmes hydrauliques** | 2 jours | Sur site ou à distance |
| **Module Vehicle : aéro & thermique** | 3 jours | Sur site ou à distance |
| **Cavitation avancée** | 1 jour | Module thématique |
| **Automatisation Python / scripts** | 1 jour | Sur site ou à distance |
| **Formation sur cas client** | sur mesure | À définir |

→ [Demander un programme détaillé](/contact)

---

## Études d'expertise

Vous n'avez pas (encore) Simerics ? Vous avez ponctuellement besoin d'une étude CFD ?

Simerics France réalise des **études clé en main** sur les domaines suivants :

- Caractérisation de pompes (rendement, ondulation, cavitation)
- Optimisation de circuits hydrauliques
- Études aérodynamiques et thermiques véhicule
- Études marines (carène, propulsion)
- Études de faisabilité avant acquisition de licence

→ [Décrire votre besoin](/contact)
